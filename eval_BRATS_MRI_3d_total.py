...
if __name__ == "__main__":
    ...
    for lamb in lamb_list:
        for rho in rho_list:
            ...
            save_root = Path(
                f"./results/{config_name}/{problem}/{mask_type}/acc{acc_factor}/lamb{lamb}/rho{rho}/{vol}"
            )
            ...
            pc_fouriercs = controllable_generation_TV.get_pc_radon_ADMM_TV_mri(
                sde,
                predictor,
                corrector,
                inverse_scaler,
            ...
            for i, recon_img in enumerate(under_img):
                plt.imsave(
                    save_root / "input" / f"{count}.png",
                    clear(under_img[i]),
                    cmap="gray",
                )
                plt.imsave(
                    save_root / "label" / f"{count}.png", clear(img[i]), cmap="gray"
                )
                count += 1
            ...
            for i, recon_img in enumerate(x):
                ...
                plt.imsave(
                    save_root / "recon" / f"{count}.png", clear(recon_img), cmap="gray"
                )
                np.save(
                    str(save_root / "input" / f"{count}.npy"),
                    clear(under_img[i], normalize=False),
                )
                np.save(
                    str(save_root / "recon" / f"{count}.npy"),
                    clear(x[i], normalize=False),
                )
                np.save(
                    str(save_root / "label" / f"{count}.npy"),
                    clear(img[i], normalize=False),
                )
                count += 1
