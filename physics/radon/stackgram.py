...
class Stackgram(nn.Module):
    ...
    def __init__(
        self, out_size, theta=None, circle=True, mode="nearest", dtype=torch.float
    ):
    ...
    def forward(self, x):
        ...
        for i_theta in range(len(self.theta)):
            ...
            linogram = grid_sample(
                repline,
                self.all_grids[i_theta].repeat(x.shape[0], 1, 1, 1).to(x.device),
                mode=self.mode,
            )
    ...
    def _create_grids(self, angles, grid_size):
        ...
        for i_theta in range(len(angles)):
            ...
            R = torch.tensor(
                [[t.sin(), t.cos(), 0.0], [t.cos(), -t.sin(), 0.0]], dtype=self.dtype
            ).unsqueeze(0)
...
class IStackgram(nn.Module):
    def __init__(
        self, out_size, theta=None, circle=True, mode="bilinear", dtype=torch.float
    ):
